package utils;

public class template {
    public static String wlcpage(){



        
       return "<!DOCTYPE html>\r\n" + //
                        "<html lang=\"en\">\r\n" + //
                        "<head>\r\n" + //
                        "    <meta charset=\"UTF-8\">\r\n" + //
                        "    <meta name=\"viewport\" content=\"width=device-width, initial-scale=1.0\">\r\n" + //
                        "    <title>Document</title>\r\n" + //
                        "\r\n" + //
                        "    <style>\r\n" + //
                        "        body {\r\n" + //
                        "            margin: 0px auto;\r\n" + //
                        "            font-family: verdana;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #container {\r\n" + //
                        "            width: 90%;\r\n" + //
                        "            margin: 0px auto;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #header {\r\n" + //
                        "            border: 1px solid #ccc;\r\n" + //
                        "            border-radius: 9px;\r\n" + //
                        "            padding: 5px;\r\n" + //
                        "            margin-bottom: 10px;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #menu {\r\n" + //
                        "            border: 1px solid #ccc;\r\n" + //
                        "            border-radius: 9px;\r\n" + //
                        "            padding: 5px;\r\n" + //
                        "            margin-bottom: 10px;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #main_body {\r\n" + //
                        "            border: 1px solid #ccc;\r\n" + //
                        "            border-radius: 9px;\r\n" + //
                        "            padding: 5px;\r\n" + //
                        "            margin-bottom: 10px;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #footer {\r\n" + //
                        "            border: 1px solid #ccc;\r\n" + //
                        "            border-radius: 9px;\r\n" + //
                        "            padding: 5px;\r\n" + //
                        "            margin-bottom: 10px;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #text {\r\n" + //
                        "            border: 1px solid #ccc;\r\n" + //
                        "            padding: 10px;\r\n" + //
                        "            border-radius: 9px;\r\n" + //
                        "            width: 90%;\r\n" + //
                        "            margin: 20px auto;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        li {\r\n" + //
                        "            list-style: none;\r\n" + //
                        "            display: inline;\r\n" + //
                        "            margin-right: 10px;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        li a {\r\n" + //
                        "            text-decoration: none;\r\n" + //
                        "            border: 1px solid #eee;\r\n" + //
                        "            padding: 5px 12px;\r\n" + //
                        "            font-family: verdana;\r\n" + //
                        "        }\r\n" + //
                        "\r\n" + //
                        "        #logo_text {\r\n" + //
                        "            display: inline-block;\r\n" + //
                        "        }\r\n" + //
                        "    </style>\r\n" + //
                        "</head>\r\n" + //
                        "<body>\r\n" + //
                        "    <div id=\"container\">\r\n" + //
                        "        <div id=\"header\">\r\n" + //
                        "            <img height=\"60\" src=\"data:image/jpeg;base64,\" alt=\"\">\r\n" + //
                        "            <h1 id=\"logo_text\">ISRDC Jabalpur</h1>\r\n" + //
                        "        </div>\r\n" + //
                        "        <div id=\"menu\">\r\n" + //
                        "            <ul>\r\n" + //
                        "                <li><a href=\"#\">Home</a></li>\r\n" + //
                        "                <li><a href=\"#\">Products</a></li>\r\n" + //
                        "                <li><a href=\"#\">Services</a></li>\r\n" + //
                        "                <li><a href=\"#\">Contact Us</a></li>\r\n" + //
                        "            </ul>\r\n" + //
                        "        </div>\r\n" + //
                        "        <div id=\"main_body\">\r\n" + //
                        "            <p id=\"text\">\r\n" + //
                        "                Lorem ipsum dolor sit amet consectetur, adipisicing elit. In atque dignissimos ut. Voluptatem consequuntur nulla ut et voluptatibus totam. Commodi maiores vel eum aperiam saepe a minus omnis consequatur dicta!\r\n" + //
                        "                Autem praesentium fugit placeat? Molestias laborum, asperiores, cum at eligendi tempora, tenetur corporis placeat cupiditate sapiente vel voluptatum temporibus quaerat esse suscipit eaque labore mollitia adipisci porro? Voluptates, dolorum saepe?\r\n" + //
                        "                Voluptas aspernatur ipsa doloremque doloribus, nam non velit! Quidem quae nobis fugit, commodi impedit repudiandae deserunt veritatis nulla facere ab optio dolore a asperiores exercitationem, rerum quod harum, facilis aut?\r\n" + //
                        "                Hic eligendi ipsum non repellat qui, corrupti distinctio eaque facilis architecto error, aperiam nemo, nam ratione. Nobis perspiciatis nulla illum molestiae laboriosam consectetur, molestias iste repellendus neque, voluptate ex est.\r\n" + //
                        "                Quae corporis repudiandae nisi unde quos ut. Omnis culpa aliquid natus nulla illo ut iure, atque temporibus. Quam quidem, repellendus qui molestiae deleniti iure excepturi. Est fuga sed officiis laborum.\r\n" + //
                        "                Eos possimus consectetur quibusdam assumenda dicta fuga, exercitationem consequuntur iusto? Fugiat explicabo temporibus optio nobis, ipsa tempora dolorum ad quisquam quam vel cupiditate eius blanditiis. Sapiente est incidunt perspiciatis deleniti!\r\n" + //
                        "                Saepe nam ipsam vero veniam tempora sapiente reprehenderit ea nihil provident amet. Accusantium alias quibusdam recusandae natus autem possimus molestias nemo, vero animi ad. Non ex neque vitae architecto optio?\r\n" + //
                        "                Sed tempore eveniet, odit sequi eius excepturi deleniti reprehenderit maiores accusantium eos est vel nesciunt provident libero cumque fuga impedit blanditiis et ipsum expedita illum nihil ullam recusandae itaque! Sint.\r\n" + //
                        "                Amet mollitia voluptatibus praesentium pariatur odio laboriosam explicabo neque adipisci, harum reprehenderit minus doloribus architecto dolorem. Reiciendis minima, culpa placeat voluptate voluptates vitae esse exercitationem numquam sapiente vero cumque dolor!\r\n" + //
                        "                Atque asperiores id repellendus, quod totam similique ratione numquam doloribus voluptate sequi enim facilis, molestiae explicabo, dignissimos ipsam eum ab quos animi consectetur dolore molestias fugiat tempora illo. Fugit, aperiam?\r\n" + //
                        "                Doloremque esse nemo nisi nostrum dolores, unde debitis maxime doloribus expedita veniam! Id perferendis nisi autem earum, hic illum cum nam provident, tenetur dolorem iure voluptate ab odio deleniti et.\r\n" + //
                        "                Qui, expedita totam! Sint dolor, magni natus enim tenetur praesentium sequi laboriosam vel asperiores quae esse, eum odio sunt iure quia consequuntur ut! Nobis adipisci, voluptatem optio veritatis repudiandae est.\r\n" + //
                        "                Adipisci odio nesciunt dolorem voluptates fuga. Tempore error dolorem nihil sint maxime corporis non porro culpa fuga corrupti dignissimos temporibus quidem vitae harum quisquam, ipsa similique. Praesentium dolores sit repellendus.\r\n" + //
                        "                Sit facilis officiis est cupiditate libero totam eius quam molestiae ipsam, quibusdam ea, adipisci culpa nesciunt fuga reiciendis, excepturi vel necessitatibus vitae. Sit dolorem ad assumenda et velit quisquam omnis?\r\n" + //
                        "                Modi distinctio laborum atque eveniet dignissimos suscipit cumque? Quidem quo recusandae quis consequuntur iste, voluptate error magni quam, molestiae dolorum numquam odit iure. Non suscipit vel animi amet tempore magni!\r\n" + //
                        "                Consequatur praesentium ducimus temporibus inventore facere, minus molestias amet qui fuga saepe, provident laborum sit consequuntur reiciendis nulla necessitatibus officiis rerum adipisci eligendi. Explicabo minima temporibus officia itaque? Error, accusantium.\r\n" + //
                        "                Aperiam, rerum. Autem optio voluptatem nemo sint natus enim. Assumenda reiciendis illum sunt rem eaque adipisci, animi sequi enim dolorem harum placeat officiis impedit blanditiis dicta deleniti doloribus dolores minus?\r\n" + //
                        "                Voluptatum ex numquam animi molestias quos aliquam sequi ullam assumenda, doloribus harum quasi dolores, corporis ratione omnis debitis odit? Autem hic asperiores repellendus eveniet assumenda corrupti minus nisi sapiente cum.\r\n" + //
                        "                Tenetur error natus ducimus voluptatibus iure accusamus odio deserunt culpa, cupiditate, quasi, fugiat vel. Est, voluptate. Ratione laudantium, sapiente quo aliquid doloremque nostrum eum blanditiis dolore distinctio quaerat, consequatur molestiae.\r\n" + //
                        "                Nemo culpa tenetur accusamus quasi earum, nostrum reprehenderit blanditiis totam eligendi, autem at veritatis, nesciunt provident quo odio voluptas nihil quia sit. Quia, quis placeat quae reprehenderit dicta excepturi. Consequuntur.\r\n" + //
                        "                Ex recusandae reiciendis, non, eos distinctio vel sed modi eaque ea nemo consectetur quae qui accusamus sapiente libero amet culpa minima veritatis! Hic, deserunt ab libero quaerat cumque quibusdam totam.\r\n" + //
                        "                Dolores ab possimus, recusandae adipisci quod ipsa! Ex blanditiis ipsa, recusandae voluptate, ut reiciendis, tenetur quibusdam sequi dolorem culpa nulla atque ea obcaecati. Debitis corrupti facilis laboriosam dolore explicabo veniam!\r\n" + //
                        "                Vitae modi itaque beatae at explicabo voluptates, voluptatum laudantium, quasi necessitatibus maiores minima unde rerum accusamus quam aspernatur laboriosam delectus fuga repellat assumenda labore quaerat cupiditate porro? Voluptatibus, velit deleniti?\r\n" + //
                        "                Quisquam tenetur, perferendis provident sit illum consectetur ex iure ipsum dicta minima doloremque perspiciatis explicabo voluptatem dolor amet maiores distinctio omnis. Incidunt suscipit in autem dolorem quis assumenda exercitationem animi.\r\n" + //
                        "                Tempora, culpa corporis quam repellat dolore placeat vitae excepturi nulla dicta ipsam blanditiis tenetur dolorem cum quasi cupiditate sit fuga impedit soluta, alias dolores. Labore voluptatem beatae repudiandae temporibus omnis!\r\n" + //
                        "                Fugit iusto hic quo at maxime quibusdam perferendis tenetur alias atque, ut cum ducimus assumenda blanditiis dolorem esse voluptas architecto voluptates! Delectus, mollitia! Laboriosam numquam placeat asperiores reprehenderit, eveniet optio?\r\n" + //
                        "                Quibusdam repellat, at eos modi soluta quaerat nesciunt illum minima quia libero asperiores a labore totam quas nostrum culpa quo tenetur dolorem, quam temporibus. Laboriosam illo hic ad sed? Omnis.\r\n" + //
                        "                Inventore, tempora vel adipisci ad non laboriosam illum omnis voluptatum illo cupiditate, distinctio totam excepturi officiis suscipit culpa explicabo cumque molestiae eaque, enim veritatis deserunt nostrum accusamus natus. Porro, incidunt.\r\n" + //
                        "                Cupiditate sit aspernatur cumque hic, perferendis veritatis beatae. Reprehenderit nesciunt facere provident doloremque atque, distinctio voluptatum sequi necessitatibus? Vero pariatur officia harum ad ratione neque, quibusdam itaque. Laboriosam, quas consequuntur!\r\n" + //
                        "                Voluptate voluptatem dolor temporibus eum assumenda obcaecati dolore fugiat sunt doloribus dolorum, earum modi cum consequuntur harum magni, reiciendis sequi recusandae quam quasi vel iure? Quibusdam, quaerat. Quod, corrupti eligendi!\r\n" + //
                        "            </p>\r\n" + //
                        "        </div>\r\n" + //
                        "        <div id=\"footer\">\r\n" + //
                        "            &copy; ISRDC Jabalpur since 765 AD\r\n" + //
                        "        </div>\r\n" + //
                        "    </div>\r\n" + //
                        "</body>\r\n" + //
                        "</html>";
    }
}


